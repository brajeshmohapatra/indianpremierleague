...
def analytics():
    ...
    if request.method == 'POST':
        ...
        venue, toss_winner, toss_decision, winner, result, method, innings_1_bat_team, innings_2_bat_team, year, result_margin, playoff, knockout, final, innings_1_score, innings_1_wickets, innings_2_score, innings_2_wickets, operator_year, operator_result_margin, operator_innings_1_score, operator_innings_1_wickets, operator_innings_2_score, operator_innings_2_wickets, operator_venue, operator_toss_winner, operator_toss_decision, operator_winner, operator_result, operator_method, operator_innings_1_bat_team, operator_innings_2_bat_team, operator_playoff, operator_knockout, operator_final = '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''

        year = request.form['year']
        operator_year = request.form['operator_year']
        venue = request.form['venue']
        if venue != '':
            operator_venue = '='
        innings_1_bat_team = request.form['innings_1_bat_team']
        if innings_1_bat_team != '':
            operator_innings_1_bat_team = '='
        innings_2_bat_team = request.form['innings_2_bat_team']
        if innings_2_bat_team != '':
            operator_innings_2_bat_team = '='
        toss_winner = request.form['toss_winner']
        if toss_winner != '':
            operator_toss_winner = '='
        toss_decision = request.form['toss_decision']
        if toss_decision != '':
            operator_toss_decision = '='
        ...
        if result != '':
            operator_result = '='
        result_margin = request.form['result_margin']
        operator_result_margin = request.form['operator_result_margin']
        method = request.form['method']
        if method != '':
            operator_method = '='
        innings_1_score = request.form['innings_1_score']
        operator_innings_1_score = request.form['operator_innings_1_score']
        innings_1_wickets = request.form['innings_1_wickets']
        operator_innings_1_wickets = request.form['operator_innings_1_wickets']
        innings_2_score = request.form['innings_2_score']
        operator_innings_2_score = request.form['operator_innings_2_score']
        innings_2_wickets = request.form['innings_2_wickets']
        operator_innings_2_wickets = request.form['operator_innings_2_wickets']
        playoff = request.form['playoff']
        if playoff != '':
            operator_playoff = '='
        knockout = request.form['knockout']
        if knockout != '':
            operator_knockout = '='
        ...
        categorical_columns = {'venue' : venue, 'toss_winner' : toss_winner, 'toss_decision' : toss_decision, 'winner' : winner, 'result' : result, 'method' : method, 'innings_1_bat_team' : innings_1_bat_team, 'innings_2_bat_team' : innings_2_bat_team}

        numerical_columns = {'year' : year, 'result_margin' : result_margin, 'innings_1_score' : innings_1_score, 'innings_1_wickets' : innings_1_wickets, 'innings_2_score' : innings_2_score, 'innings_2_wickets' : innings_2_wickets, 'playoff' : playoff, 'knockout' : knockout, 'final' : final}

        categorical_columns['venue'] = venue
        categorical_columns['toss_winner'] = toss_winner
        categorical_columns['toss_decision'] = toss_decision
        categorical_columns['winner'] = winner
        categorical_columns['result'] = result
        categorical_columns['method'] = method
        categorical_columns['innings_1_bat_team'] = innings_1_bat_team
        categorical_columns['innings_2_bat_team'] = innings_2_bat_team

        numerical_columns['year'] = year
        numerical_columns['result_margin'] = result_margin
        numerical_columns['innings_1_score'] = innings_1_score
        numerical_columns['innings_1_wickets'] = innings_1_wickets
        numerical_columns['innings_2_score'] = innings_2_score
        numerical_columns['innings_2_wickets'] = innings_2_wickets
        numerical_columns['playoff'] = playoff
        numerical_columns['knockout'] = knockout
        numerical_columns['final'] = final
        ...
        numerical_query_keys = [key for key in numerical_columns if numerical_columns[key] != '']
        numerical_query_values = [numerical_columns[key] for key in numerical_columns if numerical_columns[key] != '']
        ...
        operators_list = [operator_year, operator_venue, operator_innings_1_bat_team, operator_innings_2_bat_team, operator_toss_winner, operator_toss_decision, operator_winner, operator_result, operator_result_margin, operator_method, operator_innings_1_score, operator_innings_1_wickets, operator_innings_2_score, operator_innings_2_wickets, operator_playoff, operator_knockout, operator_final]

        operators = [op for op in operators_list if op != '']
        ...
        df = df[['date', 'venue', 'team1', 'team2', 'toss_winner', 'toss_decision', 'innings_1_bat_team', 'innings_1_score', 'innings_1_wickets', 
         'innings_2_bat_team', 'innings_2_score', 'innings_2_wickets', 'winner', 'result','result_margin', 'player_of_match', 'method', 
         'playoff', 'knockout', 'final']]
        innings_1_score, innings_2_score, won_by, stage = [], [], [], []
        for i in df.index:
            if df['innings_1_wickets'][i] != 10:
                innings_1_score.append(str(int(df['innings_1_score'][i])) + '-' + str(int(df['innings_1_wickets'][i])))
            else:
                innings_1_score.append(str(int(df['innings_1_score'][i])))
        df['innings_1_score'] = innings_1_score
        df.drop('innings_1_wickets', axis = 1, inplace = True)

        for i in df.index:
            if df['innings_2_wickets'][i] != 10:
                innings_2_score.append(str(int(df['innings_2_score'][i])) + '-' + str(int(df['innings_2_wickets'][i])))
            else:
                innings_2_score.append(str(int(df['innings_2_score'][i])))
        df['innings_2_score'] = innings_2_score
        df.drop('innings_2_wickets', axis = 1, inplace = True)

        for i in df.index:
            won_by.append(str(int(df['result_margin'][i])) + ' ' + str(df['result'][i]))
        df['result'] = won_by
        df.drop('result_margin', axis = 1, inplace = True)

        for i in df.index:
            if df['final'][i] == 1:
                stage.append('Final')
            elif df['playoff'][i] == 1 and df['final'][i] == 0:
                stage.append('Playoffs')
            else:
                stage.append('League')
        df['Stage'] = stage
        df.drop(['playoff', 'knockout', 'final'], axis = 1, inplace = True)

        df.columns = ['Date', 'Venue', '1st Team', '2nd Team', 'Toss Winner', 'Toss Decision', 'First Batting Team', '1st Innings Score', 
              'Second Batting Team', '2nd Innings Score', 'Winner', 'Won By', 'Man of the Match', 'Methods Applied', 'Stage']
        return render_template('analytics.html', tables = [df.to_html(classes = 'data', table_id = 'table', index = False)], titles = df.columns.values)
    else:
        ...
        if innings == 1:
            ...
            if ('Lucknow Super Giants' in teams) or ('Gujarat Titans' in teams):
                ...
                if overs > 4: 
                    ...
                    return render_template('predict_1_a_5.html', prediction_text = 'Based on the historical data, current score and playing conditions at the moment, {} can score somewhere around {}'.format(batting_team, runs + prediction), batting_team = batting_team, bowling_team = bowling_team, venue = venue, runs = runs, wickets = wickets, runs_last_5_overs = runs_last_5_overs, wickets_last_5_overs = wickets_last_5_overs, toss_winner = toss_winner, overs = str(overs) + '.' + str(balls), innings = innings)
                    
                else:
                    return render_template('predict_1_b_5.html', prediction_text = 'Please wait till 5 overs to complete for better result. Thanks for your patience.', batting_team = batting_team, bowling_team = bowling_team, venue = venue, runs = runs, wickets = wickets, toss_winner = toss_winner, overs = str(overs) + '.' + str(balls), innings = innings)                
            ...
            if ('Lucknow Super Giants' in teams) or ('Gujarat Titans' in teams):
               ...
               'Sunrisers Hyderabad' : '#FF6600'}
                    ...
                    plt.clf()
                    return render_template('predict_2_a_5.html', prediction_text = 'Based on the historical data, current score and playing conditions at the moment', batting_team = batting_team, bowling_team = bowling_team, venue = venue, runs = runs, wickets = wickets, runs_last_5_overs = runs_last_5_overs, wickets_last_5_overs = wickets_last_5_overs, toss_winner = toss_winner, overs = str(overs) + '.' + str(balls), innings = innings)
                else:
                    return render_template('predict_2_b_5.html', prediction_text = 'Please wait till 5 overs to complete for better result. Thanks for your patience.', batting_team = batting_team, bowling_team = bowling_team, venue = venue, runs = runs, wickets = wickets, toss_winner = toss_winner, overs = str(overs) + '.' + str(balls), innings = innings)                
